...
5. evidence_source 必须同时包含材料名称和原文片段，格式为"【材料名称】原文内容"，确保任何判断可被人工逐条回溯复核。
6. confidence 仅允许三个小写值："high"、"medium"、"low"，不允许大写、全拼或其他变体。
   - high：原文直接、明确支撑该标准，无需推断。
   - medium：存在合理推断空间，需人工复核确认。
   - low：缺乏直接证据，仅陈列材料片段，禁止生成任何推断。
7. 输出JSON的层级结构和字段名称必须与下方模板完全一致，不得增减字段。
